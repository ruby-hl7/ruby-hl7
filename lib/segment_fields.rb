...
module HL7::Message::SegmentFields
  ...
  def []=( index, value )
    @elements[index] = HL7.configuration.preserve_data_types ? value : value.to_s
  end
  ...
  def read_field( name ) #:nodoc:
    ...
    ret = @elements[ idx ]
    if !HL7.configuration.enable_repetitions && (ret.kind_of?(Array) && ret.length == 1)
      ret = ret.first
    end
  ...
  def write_field( name, value ) #:nodoc:
    ...
    @elements[ idx ] = HL7.configuration.preserve_data_types ? value : value.to_s
  end
end
