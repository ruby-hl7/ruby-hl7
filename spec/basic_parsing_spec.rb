...
describe HL7::Message do
  context 'basic parsing' do
    ...
    it 'parses delimiters properly' do
      msg = described_class.new(@base_msh)
      expect(msg.element_delim).to eq '|'
      expect(msg.item_delim).to eq '^'

      msg = described_class.new(@base_msh_alt_delims)
      expect(msg.element_delim).to eq '$'
      expect(msg.item_delim).to eq '@'
    end

    it 'parses via the constructor' do
      msg = described_class.new(@simple_msh_txt)
      expect(msg.to_hl7).to eq @simple_msh_txt
    end

    it 'parses via the class method' do
      msg = described_class.parse(@simple_msh_txt)
      expect(msg.to_hl7).to eq @simple_msh_txt
    end

    it 'only parses String and Enumerable data' do
      expect do
        described_class.parse :MSHthis_shouldnt_parse_at_all
      end.to raise_error(HL7::ParseError)
    end

    it 'parses empty strings' do
      expect { described_class.new @empty_txt }.not_to raise_error
    end
    ...
    it 'parses Enumerable data' do
      ...
      msg = described_class.new(test_file)
      expect(msg.to_hl7).to eq @simple_msh_txt
    end
    ...
    it 'can parse its own MLLP output' do
      ...
      expect(msg).not_to be_nil
      expect do
        post_mllp = described_class.parse(msg.to_mllp)
    ...
    it 'can parse an empty segment' do
      expect { described_class.new @empty_segments_txt }.not_to raise_error
    end
  end
end
